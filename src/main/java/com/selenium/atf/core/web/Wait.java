...
public class Wait<T extends Component<T>> {
	...
	private static final int DEFAULT_RETRAY_DELAY = 500;
	private T component;
	...
	public T toBeAvailable() {
		...
		if (!this.component.isAvailable()) {
			throw new TimeoutException("Time out after " + DEFAULT_TIMEOUT
					+ "ms. waiting for" + " '"
	...
	private int delay() {
		try {
			Thread.sleep(DEFAULT_RETRAY_DELAY);
			return DEFAULT_RETRAY_DELAY;
